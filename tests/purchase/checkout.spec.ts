...
type InfoScenario = { label: string; skipInput: keyof CheckoutInfoData; expectedError: CheckoutInfoError };
...
for (const persona of AUTHENTICATED) {
  test.describe(`${persona.role}`, { tag: persona.tag }, () => {
    ...
    (
      [
        { label: 'first name', skipInput: 'firstName', expectedError: t.checkout.info.errors.firstName },
        { label: 'last name', skipInput: 'lastName', expectedError: t.checkout.info.errors.lastName },
        { label: 'zip code', skipInput: 'zipCode', expectedError: t.checkout.info.errors.zipCode },
    ...
    if (persona.isBaseline) {
      test('visual: checkout', { tag: '@visual' }, async ({ page, act }) => {
        await test.step('⬜ Add items and submit checkout info', async () => {
        ...
        await expect(page, '🟧 UI: Layout visual check').toHaveScreenshot('checkout-summary-page.png', {
          fullPage: true,
        });
        ...
        await expect(page, '🟧 UI: Layout visual check').toHaveScreenshot('checkout-complete-page.png', {
          fullPage: true,
        });
      });
    }
